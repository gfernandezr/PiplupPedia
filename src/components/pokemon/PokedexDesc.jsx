...
function PokedexDesc({ entries, language }) {
  ...
  console.log(es_entries);
  return (
    <div className="flex flex-col gap-3 p-1">
      {es_entries.map((e, index) => (
        <div className={`flex flex-col rounded-md bg-[${versions[e.version.name].color}]
        md:flex-row`}>
          <div className="flex flex-row justify-evenly p-2
          md:w-80">
            <img src={versions[e.version.name].icon} className="w-fit h-fit my-auto" />
            <b className="my-auto text-gray-400">{versions[e.version.name].name_es}</b>
          </div>
